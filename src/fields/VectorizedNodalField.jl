...
Base.axes(::VectorizedNodalField{T, N, NF, NN, V}) where {T, N, NF, NN, V <: DenseArray} = (Base.OneTo(NF), Base.OneTo(NN))
...
function Base.getindex(field::VectorizedNodalField{T, N, NF, NN, V}, d::Int, n::Int) where {T, N, NF, NN, V <: DenseArray}
...
function Base.setindex!(field::VectorizedNodalField{T, N, NF, NN, V}, v, d::Int, n::Int) where {T, N, NF, NN, V <: DenseArray}
  @assert d > 0 && d <= num_fields(field)
  @assert n > 0 && n <= num_nodes(field)
  setindex!(field.vals, v, (n - 1) * num_fields(field) + d)
end
Base.size(::VectorizedNodalField{T, N, NF, NN, V}) where {T, N, NF, NN, V <: DenseArray} = (NF, NN)
