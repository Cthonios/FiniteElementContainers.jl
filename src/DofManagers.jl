function setup_hessian_coordinates(
  fspaces::Vector{FunctionSpace{N, D, L, Itype, Ftype}},
...
function setup_hessian_bc_mask(
  fspace::FunctionSpace{N, D, L, Itype, Ftype},
  ids::Matrix{<:Integer}, is_bc::Matrix{<:Integer}
) where {N, D, L, Itype, Ftype}

  n_els = length(fspace)  
  n_nodes_per_element = size(fspace.conn, 1)
  n_fields = size(ids, 1)
...
get_bc_values(d::DofManager, U::A) where A <: AbstractArray = U[d.is_bc]
get_unknown_size(d::DofManager) = sum(d.is_unknown)
get_unknown_values(d::DofManager, U::A) where A <: AbstractArray = U[d.is_unknown]
...
function DofManager(
  ...
  return DofManager(
    field_shape, is_bc, is_unknown, 
    ids, bc_indices, dof_to_unknown,
