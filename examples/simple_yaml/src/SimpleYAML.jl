...
# """
# Tagged-union value node.  Every YAML value is one of the seven tags below.
# Using a concrete struct with a tag enum keeps things type-stable for the
# compiler even though the result tree is heterogeneous.
# """
# @enum YAMLTag begin
#     TAG_NULL
#     TAG_BOOL
...
# struct YAMLValue
#     tag   :: YAMLTag
#     # Scalar storage (only one is used at a time)
#     bval  :: Bool
#     ival  :: Int64
#     fval  :: Float64
#     sval  :: String
...
# YAMLString(s::AbstractString)= YAMLValue(TAG_STRING,false, 0,   0.0, String(s),   YAMLValue[], Dict{String,YAMLValue}())
...
#     v.tag === TAG_BOOL || error("YAMLValue is not a bool (tag=$(v.tag))")
#     return v.bval
# end
# function as_int(v::YAMLValue)::Int64
#     v.tag === TAG_INT || error("YAMLValue is not an int (tag=$(v.tag))")
#     return v.ival
# end
# function as_float(v::YAMLValue)::Float64
#     v.tag === TAG_FLOAT || error("YAMLValue is not a float (tag=$(v.tag))")
#     return v.fval
# end
# function as_string(v::YAMLValue)::String
#     v.tag === TAG_STRING || error("YAMLValue is not a string (tag=$(v.tag))")
#     return v.sval
# end
# function as_array(v::YAMLValue)::Vector{YAMLValue}
#     v.tag === TAG_ARRAY || error("YAMLValue is not an array (tag=$(v.tag))")
#     return v.arr
# end
# function as_dict(v::YAMLValue)::Dict{String,YAMLValue}
#     v.tag === TAG_DICT || error("YAMLValue is not a dict (tag=$(v.tag))")
#     return v.dict
# end
...
#     lines   :: Vector{String}
#     lineno  :: Int
#     anchors :: Dict{String, YAMLValue}   # anchor name → value
# end

# ParseState(src::String) = ParseState(split(src, '\n'), 1, Dict{String,YAMLValue}())

# # ── Low-level line helpers ────────────────────────────────────────────────────

# @inline at_end(ps::ParseState) = ps.lineno > length(ps.lines)

# @inline current_line(ps::ParseState) =
#     ps.lineno <= length(ps.lines) ? ps.lines[ps.lineno] : ""

# function peek_indent(ps::ParseState)
#     # Look ahead past blank / comment lines to find the indent of the next
#     # content line.  Returns -1 at EOF.
#     i = ps.lineno
#     while i <= length(ps.lines)
#         l = ps.lines[i]
#         s = lstrip(l)
#         if !isempty(s) && s[1] != '#'
#             return ncodeunits(l) - ncodeunits(s)   # byte-count indent
#         end
#         i += 1
#     end
#     return -1
...
# """Skip blank lines and comment-only lines, advancing ps.lineno."""
...
# """Return the indent (number of leading spaces) of a line."""
# function line_indent(l::String)
#     i = 1
#     n = ncodeunits(l)
#     while i <= n && codeunit(l, i) == UInt8(' ')
#         i += 1
#     end
#     return i - 1
# end

# """Strip inline comment from the end of a scalar string."""
# function strip_inline_comment(s::AbstractString)
#     # A comment is ' #...' — space then hash
#     i = 1
#     n = ncodeunits(s)
#     while i <= n
#         c = codeunit(s, i)
#         if c == UInt8('#') && i > 1 && codeunit(s, i-1) == UInt8(' ')
#             return rstrip(s[1:i-2])
#         end
#         i += 1
#     end
#     return s
...
# """
# Parse a YAML bare scalar string into a typed YAMLValue.
# Handles null, bool, int (decimal/hex/octal/binary), float, and string fallback.
# """
# function parse_scalar(s::AbstractString)::YAMLValue
...
#     # integers  (0x… hex, 0o… octal, 0b… binary, plain decimal)
#     if length(s) >= 3 && s[1] == '0' && s[2] == 'x'
#         v = tryparse(Int64, s[3:end]; base=16)
#         v !== nothing && return YAMLInt(v)
#     elseif length(s) >= 3 && s[1] == '0' && s[2] == 'o'
#         v = tryparse(Int64, s[3:end]; base=8)
#         v !== nothing && return YAMLInt(v)
#     elseif length(s) >= 3 && s[1] == '0' && s[2] == 'b'
#         v = tryparse(Int64, s[3:end]; base=2)
#         v !== nothing && return YAMLInt(v)
#     else
#         v = tryparse(Int64, s)
#         v !== nothing && return YAMLInt(v)
...
#     fv !== nothing && return YAMLFloat(fv)
...
# """
# Parse a single-quoted YAML string starting just after the opening quote.
# Returns (String, new_index) where new_index points past the closing quote.
# """
# function parse_single_quoted(s::AbstractString, start::Int)::Tuple{String,Int}
#     buf = IOBuffer()
#     i = start
#     n = ncodeunits(s)
#     while i <= n
...
# """
# Parse a double-quoted YAML string starting just after the opening quote.
# Handles standard JSON-like escape sequences plus YAML-specific \\N \\L \\P.
# """
# function parse_double_quoted(s::AbstractString, start::Int)::Tuple{String,Int}
#     buf = IOBuffer()
#     i = start
#     n = ncodeunits(s)
#     while i <= n
...
#             elseif esc == '/';  write(buf, '/')
#             elseif esc == '\\'; write(buf, '\\')
#             elseif esc == 'N';  write(buf, '\u0085')  # YAML next-line
#             elseif esc == '_';  write(buf, '\u00a0')  # YAML non-breaking space
#             elseif esc == 'L';  write(buf, '\u2028')  # YAML line-separator
#             elseif esc == 'P';  write(buf, '\u2029')  # YAML para-separator
#             elseif esc == 'x'
#                 i += 1
#                 hex = (i+1 <= n) ? s[i:i+1] : ""
#                 cp = tryparse(UInt32, hex; base=16)
#                 cp !== nothing ? write(buf, Char(cp)) : write(buf, '?')
#                 i += 1
#             elseif esc == 'u'
#                 i += 1
#                 hex = (i+3 <= n) ? s[i:i+3] : ""
...
# `header` is the rest of the line after the indicator, e.g. "|-", "|2", "|+2".
# Returns the assembled String.
# """
# function parse_block_scalar(ps::ParseState, style::Char, header::AbstractString)::String
#     # Parse chomping indicator and explicit indent
#     chomping = :clip    # clip | strip (-) | keep (+)
#     explicit_indent = 0

#     rest = strip(header)
#     # strip trailing comment
#     ci = findfirst('#', rest)
#     ci !== nothing && (rest = rstrip(rest[1:ci-1]))

#     i = 1
#     while i <= length(rest)
#         c = rest[i]
#         if c == '-';       chomping = :strip; i += 1
#         elseif c == '+';   chomping = :keep;  i += 1
#         elseif isdigit(c)
#             explicit_indent = parse(Int, string(c))
#             i += 1
#         else
#             break
#         end
...
#     # The header line has already been consumed by the caller.
...
#         # Empty lines are kept (as "") regardless of indent
#         if isempty(content)
#             push!(lines_out, "")
#             ps.lineno += 1
#             continue
...
#         # A line with less indent than block_indent ends the scalar
#         indent < block_indent && break

#         push!(lines_out, l[block_indent+1:end])
#         ps.lineno += 1
#     end
...
#     else
#         # Folded: fold non-empty lines separated by single newlines;
...
#     elseif chomping === :clip
#         # Ensure exactly one trailing newline
#         result = rstrip(result, '\n') * "\n"
#     else  # keep — preserve all trailing newlines
#         # already correct
#     end
...
# """
# Parse a flow (inline) value starting at position `i` in string `s`.
# Returns (YAMLValue, new_index).
# """
# function parse_flow_value(ps::ParseState, s::AbstractString, i::Int)::Tuple{YAMLValue,Int}
...
#         return YAMLString(str), i2
#     elseif c == '&'
#         # Anchor in flow context — parse the name then the value
#         j = i + 1
#         while j <= n && s[j] != ' ' && s[j] != ',' && s[j] != '}' && s[j] != ']'
#             j += 1
#         end
#         anchor_name = s[i+1:j-1]
#         val, i2 = parse_flow_value(ps, s, j)
#         ps.anchors[anchor_name] = val
#         return val, i2
#     elseif c == '*'
#         j = i + 1
#         while j <= n && s[j] != ' ' && s[j] != ',' && s[j] != '}' && s[j] != ']'
#             j += 1
#         end
#         alias_name = s[i+1:j-1]
#         val = get(ps.anchors, alias_name, YAMLNull)
#         return val, j
#     else
...
# function parse_flow_sequence(ps::ParseState, s::AbstractString, i::Int)::Tuple{YAMLValue,Int}
#     items = YAMLValue[]
#     n = ncodeunits(s)
#     while i <= n
#         # skip whitespace
...
# function parse_flow_mapping(ps::ParseState, s::AbstractString, i::Int)::Tuple{YAMLValue,Int}
...
#         key = (key_val.tag === TAG_STRING ? key_val.sval : string_of(key_val))
...
# function string_of(v::YAMLValue)::String
...
#     v.tag === TAG_NULL    && return "null"
#     return "???"
# end

# # ──────────────────────────────────────────────────────────────────────────────
# # Main recursive block parser
# # ──────────────────────────────────────────────────────────────────────────────

# """
# Parse a YAML node (mapping, sequence, or scalar) whose content lines all
# have indent ≥ `min_indent`.  The caller must have already skipped blanks.
...
#         # Only advance past if it's a true separator (followed by space/EOF)
#         rest = length(content) > 3 ? content[4:4] : " "
#         if rest == " " || rest == "\n" || rest == "#"
#             ps.lineno += 1
#             return parse_node(ps, min_indent)
#         end
#     end

#     # ── block sequence ────────────────────────────────────────────────────────
#     if content[1] == '-' && (length(content) == 1 || content[2] == ' ' || content[2] == '\n')
#         return parse_block_sequence(ps, indent)
#     end

#     # ── explicit key with '?' ─────────────────────────────────────────────────
#     # Not commonly needed; skip complex mapping keys for now.

#     # ── mapping or scalar ─────────────────────────────────────────────────────
#     # Try to detect a block mapping key on this line.
#     key, colon_pos = find_mapping_key(content)
#     if key !== nothing
#         return parse_block_mapping(ps, indent)
#     end
...
# """
# Determine if `content` (stripped line) begins with a mapping key.
# Returns (key_string, position_after_colon) or (nothing, 0).
# A mapping key is:  <bare-key>: …  or  "quoted": …  or  'quoted': …
# """
# function find_mapping_key(content::AbstractString)
...
#             ni = nextind(content,i)

#             if ni > lastindex(content) ||
#                content[ni] == ' ' ||
#                content[ni] == '\t' ||
#                content[ni] == '#'

#                 key = rstrip(content[firstindex(content):prevind(content,i)])

#                 isempty(key) && return nothing, 0
...
# """Parse a block mapping whose first key is on a line with indent `map_indent`."""
...
#         l = current_line(ps)
#         indent = line_indent(l)
#         indent < map_indent && break          # outdented → mapping ends
#         indent > map_indent && break          # shouldn't happen; bail
...
#         # Must be a mapping key at this indent
#         key, after_colon = find_mapping_key(content)
#         key === nothing && break

#         ps.lineno += 1   # consume key line

#         # Check for anchor on the key line, after the colon
#         anchor_name = nothing
#         rest_of_line = after_colon <= ncodeunits(content) ? lstrip(content[after_colon:end]) : ""

#         # Strip trailing comment from rest_of_line for non-block scalars
#         rest_stripped = strip_inline_comment(rstrip(rest_of_line))

#         # # Determine value
#         # val = if isempty(rest_stripped) || rest_stripped[1] == '#'
#         #     # Value is on the next line(s) — block node
#         #     next_ind = peek_indent(ps)
#         #     if next_ind > map_indent
#         #         parse_node(ps, next_ind)
#         #     else
#         #         YAMLNull
#         #     end
#         # Determine value
...
#             # Value lives on subsequent line(s)
#             next_ind = peek_indent(ps)
...
#             elseif next_ind > map_indent
#                 # Indented mapping/scalar
#                 parse_node(ps, next_ind)
...
#                 # - item
#                 #
#                 # where the sequence indicator is aligned with the key.
#                 next_line = lstrip(current_line(ps))
...
#                     parse_block_sequence(ps, next_ind)
#                 else
#                     YAMLNull
#                 end
...
#                 YAMLNull
#             end
#         elseif rest_stripped[1] == '|' || rest_stripped[1] == '>'
#             # Block scalar
#             style = rest_stripped[1]
#             header = length(rest_stripped) > 1 ? rest_stripped[2:end] : ""
#             YAMLString(parse_block_scalar(ps, style, header))
#         elseif rest_stripped[1] == '{'
#             v, _ = parse_flow_mapping(ps, rest_stripped, 2)
#             v
#         elseif rest_stripped[1] == '['
#             v, _ = parse_flow_sequence(ps, rest_stripped, 2)
#             v
#         elseif rest_stripped[1] == '&'
#             # Anchor reference
#             j = 2
#             n2 = ncodeunits(rest_stripped)
#             while j <= n2 && rest_stripped[j] != ' '
#                 j += 1
#             end
#             anchor_name2 = rest_stripped[2:j-1]
#             inner = lstrip(rest_stripped[j:end])
#             v = if isempty(inner) || inner[1] == '#'
#                 next_ind = peek_indent(ps)
#                 next_ind > map_indent ? parse_node(ps, next_ind) : YAMLNull
#             else
#                 parse_scalar(strip_inline_comment(inner))
#             end
#             ps.anchors[anchor_name2] = v
#             v
#         elseif rest_stripped[1] == '*'
#             # Alias
#             j = 2
#             n2 = ncodeunits(rest_stripped)
#             while j <= n2 && rest_stripped[j] != ' ' && rest_stripped[j] != '#'
#                 j += 1
#             end
#             alias_name = rest_stripped[2:j-1]
#             get(ps.anchors, alias_name, YAMLNull)
#         else
#             parse_scalar(rest_stripped)
#         end
...
# """Parse a block sequence whose '-' indicators are at indent `seq_indent`."""
...
#         indent > seq_indent && break   # continuation handled inside

#         content = lstrip(l)
#         isempty(content) && (ps.lineno += 1; continue)
#         content[1] != '-' && break   # not a sequence item

#         # Ensure it's '- ' or '-\n' or just '-' at end
#         if length(content) >= 2 && content[2] != ' ' && content[2] != '\t'
#             break
#         end

#         ps.lineno += 1   # consume the '-' line

#         # The item content can be inline (after '-') or on subsequent lines
#         inline = length(content) > 1 ? lstrip(content[2:end]) : ""
#         inline_stripped = strip_inline_comment(rstrip(inline))

#         val = if isempty(inline_stripped) || inline_stripped[1] == '#'
#             # Item value is indented below
#             next_ind = peek_indent(ps)
#             if next_ind > seq_indent
#                 parse_node(ps, next_ind)
#             else
#                 YAMLNull
#             end
#         elseif inline_stripped[1] == '|' || inline_stripped[1] == '>'
#             style = inline_stripped[1]
#             header = length(inline_stripped) > 1 ? inline_stripped[2:end] : ""
#             YAMLString(parse_block_scalar(ps, style, header))
#         elseif inline_stripped[1] == '{'
#             v, _ = parse_flow_mapping(ps, inline_stripped, 2)
#             v
#         elseif inline_stripped[1] == '['
#             v, _ = parse_flow_sequence(ps, inline_stripped, 2)
#             v
#         elseif inline_stripped[1] == '-' && (length(inline_stripped) == 1 || inline_stripped[2] == ' ')
#             # Nested sequence inline with the '-'
#             # Re-inject into lines at appropriate indent
#             parse_block_sequence(ps, seq_indent + 2)
#         else
#             # check if it's a mapping key
#             key, _ = find_mapping_key(inline_stripped)
#             if key !== nothing
#                 # inline mapping: treat the current rest as a new line and parse
#                 # We push a virtual line back
#                 new_indent = seq_indent + 2
#                 # fake_line = " "^new_indent * inline_stripped
#                 buf = IOBuffer()
#                 for i in 1:new_indent
#                     write(buf, ' ')
#                 end
#                 write(buf, inline_stripped)
#                 fake_line = String(take!(buf))
#                 fake_line = fake_line * inline_stripped
#                 insert!(ps.lines, ps.lineno, fake_line)
#                 parse_node(ps, new_indent)
#             else
#                 parse_scalar(inline_stripped)
#             end
#         end
...
# """
# Parse a scalar that may span multiple lines (plain multi-line scalar).
# Continuation lines must be more indented than `min_indent`.
# """
...
#     # Anchor or alias on a standalone scalar line
...
#             j += 1
#         end
#         anchor_name = content[2:j-1]
#         inner = j <= n ? lstrip(content[j:end]) : ""
#         val = isempty(inner) ? parse_node(ps, min_indent) : parse_scalar(strip_inline_comment(rstrip(inner)))
#         ps.anchors[anchor_name] = val
#         return val
#     elseif !isempty(content) && content[1] == '*'
#         ps.lineno += 1
#         j = 2
#         n = ncodeunits(content)
...
#             j += 1
#         end
#         alias_name = content[2:j-1]
#         return get(ps.anchors, alias_name, YAMLNull)
#     end
...
#         if content[1] == '"'
#             ps.lineno += 1
#             str, _ = parse_double_quoted(content, 2)
#             return YAMLString(str)
#         elseif content[1] == '\''
#             ps.lineno += 1
#             str, _ = parse_single_quoted(content, 2)
#             return YAMLString(str)
#         elseif content[1] == '|' || content[1] == '>'
#             style = content[1]
#             header = length(content) > 1 ? content[2:end] : ""
#             ps.lineno += 1   # consume the | / > header line
#             return YAMLString(parse_block_scalar(ps, style, header))
#         elseif content[1] == '{'
#             ps.lineno += 1
#             v, _ = parse_flow_mapping(ps, content, 2)
#             return v
#         elseif content[1] == '['
#             ps.lineno += 1
#             v, _ = parse_flow_sequence(ps, content, 2)
#             return v
#         end
#     end

#     # Plain multi-line scalar: first line
#     ps.lineno += 1
#     parts = [strip_inline_comment(rstrip(content))]

#     # Continuation lines (more indented, not a key, not a sequence item)
#     while !at_end(ps)
#         skip_empty!(ps)
#         at_end(ps) && break
#         l2 = current_line(ps)
...
#         isempty(c2) && break
#         c2[1] == '#' && break
#         # Don't consume what looks like a new key or sequence
#         k2, _ = find_mapping_key(c2)
#         k2 !== nothing && break
#         (c2[1] == '-' && (length(c2) == 1 || c2[2] == ' ')) && break

#         push!(parts, strip_inline_comment(rstrip(c2)))
#         ps.lineno += 1
#     end
...
# """
#     loads(yaml::String) -> YAMLValue

# Parse a YAML string and return a `YAMLValue` tree.
# """
...
# """
#     load(path::String) -> YAMLValue

# Read a YAML file from `path` and return a `YAMLValue` tree.
# """
# function load(path::String)::YAMLValue
...
# function get_value(d::Dict{String, YAMLValue}, key, ::Type{Array})
#     return as_array(d[key])
# end

# function get_value(d::Dict{String, YAMLValue}, key, ::Type{<:Dict})
#     return as_dict(d[key])
# end

# function get_value(d::Dict{String, YAMLValue}, key, ::Type{Float64})
#     return as_float(d[key])
# end

# function get_value(d::Dict{String, YAMLValue}, key, ::Type{Int})
#     return as_int(d[key])
# end

# function get_value(d::Dict{String, YAMLValue}, key, ::Type{String})
#     return as_string(d[key])
# end

# function to_dict(v::YAMLValue)
#     if v.tag === TAG_NULL
#         return nothing
#     elseif v.tag === TAG_BOOL
#         return v.bval
#     elseif v.tag === TAG_INT
#         return v.ival
#     elseif v.tag === TAG_FLOAT
#         return v.fval
#     elseif v.tag === TAG_STRING
#         return v.sval
#     elseif v.tag === TAG_ARRAY
#         vals = Any[]
#         for v in v.arr
